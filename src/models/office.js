...
import { query, get, create, update, pubOrUnpub, remove } from '../services/office'
...
export default modelExtend(pageModel, {
  ...
  subscriptions: {
    setup ({ dispatch, history }) {
      history.listen(location => {
        ...
        if (location.pathname === '/offices') {
        ...
        else if(location.pathname === '/offices/create'){
          ...
          const match = pathToRegexp('/offices/:id/:opt?').exec(location.pathname);
          //console.log(match);
          if(match){
  ...
  effects: {
    ...
    *create ({ payload }, { call, put }) {
      ...
      if (success) {
        yield put(routerRedux.push('/offices'))
      } else {
        throw res
      }
    },
    ...
    *update ({ payload }, { call, put, select }) {
      ...
      const { success, message, data } = res
      if (success) {
          const {pageSize, current, criteria} = yield select (_ => _.office.pagination)
          const query = Object.assign({page: current, ...pageSize}, criteria)
          yield put(routerRedux.push({pathname: '/offices', query: query}))
      }
      else {
        throw res
      }
